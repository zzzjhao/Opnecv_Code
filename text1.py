...
COLOR_ARRAY = [[red_min, red_max, 'red'], [red2_min, red2_max, 'red'], [green_min, green_max, 'green'],
               ...
               [yellow_min, yellow_max,
                'yellow']]  # , [ black_min, black_max, 'black'],  [ white_min, white_max, 'white']  ]
    ...
    for (color_min, color_max, name) in COLOR_ARRAY:
        ...
        if number >= 1:
            ...
            for i in range(0, number):
                ...
                print
                'NO:', i, ' size:', len(contours[i])
            if total > 400:
                print
                'Currrent color is ', name
                cv2.destroyAllWindows()
                sys.exit()

    rawCapture.truncate(0,cv2.CAP_DSHOW)
